...
def main():
    ...
    google_search_pool = st.session_state.get("google_search_pool")

    # 侧边栏设置
    with st.sidebar:
        st.header("搜索配置")
        search_topic = st.text_input("搜索主题", value="稳定币市场发展趋势与监管动态")
        ...
        topic_desc = st.text_area(
            "搜索说明",
            value=(
                "分析全球稳定币市场的发展态势，包括主流稳定币的市值变化、"
                "跨境支付与DeFi应用场景，以及各国监管政策、风险防控与合规化趋势。"
            )
        )
    ...
    if start_button:
        ...
        elif not google_api_configured or not google_search_pool or not google_search_pool.clients:
